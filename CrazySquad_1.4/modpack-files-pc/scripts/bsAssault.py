...
class AssaultGame(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        return [("Score to Win", {'minValue': 1, 'default': 3}),
                ("Time Limit", {'choices': [('None', 0),
                                            ...
                                            ('20 Minutes', 1200)],
                                'default':0}),
                ("Respawn Times", {'choices': [('Shorter', 0.25),
                                               ('Short', 0.5),
                                               ('Normal', 1.0),
                                               ('Long', 2.0),
                                               ('Longer', 4.0)],
    ...
    def onTransitionIn(self):
        bs.TeamGameActivity.onTransitionIn(
            self, music='Epic'
            if self.settings['Epic Mode'] else 'ForwardMarch')
    ...
    def onBegin(self):
        ...
        for team in self.teams:
            ...
            m.addActions(
                conditions=('theyHaveMaterial', bs.getSharedObject(
                    'playerMaterial')),
                actions=(('modifyPartCollision', 'collide', True),
                         ('modifyPartCollision', 'physical', False),
                         ('call', 'atConnect', bs.Call(
                             self._handleBaseCollide, team))))

        # create a score region and flag for each team
        for team in self.teams:
            team.gameData['basePos'] = self.getMap(
            ).getFlagPosition(team.getID())
            ...
            region = bs.newNode(
                ...
                attrs={'position': (p[0],
                       ...
                       'scale': (0.5, 0.5, 0.5),
                       'type': 'sphere',
                       'materials': [self._baseRegionMaterials
                                     [team.getID()]]})
            ...
            if bs.getGameTime() != self._lastScoreTime:
                ...
                for p in playerTeam.players:
                    if p.isAlive():
                        ...
                        bs.gameTimer(500, light.delete)
                        bs.animate(light, 'intensity', {0: 0, 100: 1.0, 500: 0})
                        p.actor.handleMessage(bs.StandMessage(
                            newPos, random.uniform(0, 360)))
