...
class ConquestGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        for i in range(len(self.getMap().flagPoints)):
            ...
            flag = ConquestFlag(position=point, touchable=False,
                                materials=[self._extraFlagMaterial])
            self.projectFlagStand(point)
            self._flags.append(flag)
            flag.id = i
            flag.light = bs.newNode('light',
                                    owner=flag.node,
                                    attrs={'position': point,
                                           'intensity': 0.25,
    ...
    def _flashFlag(self, flag, length=1000):
        light = bs.newNode('light',
                           attrs={'position': flag.node.position,
                                  'heightAttenuated': False,
