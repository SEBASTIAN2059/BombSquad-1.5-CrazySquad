...
class Icon(bs.Actor):
    ...
    def __init__(self, player, position, scale, showLives=True, showDeath=True,
        ...
        icon = player.getIcon()
        self.node = bs.newNode('image', owner=self, attrs={
            ...
            'absoluteScale': True,
            'attach': 'bottomCenter'})
        self._nameText = bs.newNode('text', owner=self.node, attrs={
            ...
            'vAttach': 'bottom'})
        if self._showLives:
            self._livesText = bs.newNode('text', owner=self.node, attrs={
                'text': 'x0',
                'color': (1, 1, 0.5),
                'hAlign': 'left',
                'vrDepth': 430,
    ...
    def setPositionAndScale(self, position, scale):
        ...
        if self._showLives:
            self._livesText.position = (
                position[0]+scale*10.0, position[1]-scale*43.0)
    ...
    def handlePlayerDied(self):
        ...
        if self._showDeath:
            bs.animate(
                self.node, 'opacity',
                {0: 1.0, 50: 0.0, 100: 1.0, 150: 0.0, 200: 1.0, 250: 0.0, 300:
                 1.0, 350: 0.0, 400: 1.0, 450: 0.0, 500: 1.0, 550: 0.2})
            lives = self._player.gameData['lives']
            if lives == 0:
...
class EliminationGame(bs.TeamGameActivity):
    ...
    def getSettings(cls, sessionType):
        settings = [("Lives Per Player", {'default': 1, 'minValue': 1,
    ...
    def getInstanceDescription(self):
        return 'Last team standing wins.' if isinstance(
            self.getSession(),
            bs.TeamsSession) else 'Last one standing wins.'
    ...
    def onPlayerJoin(self, player):
        ...
        if self.hasBegun():
            ...
            if self._getTotalTeamLives(
                    player.getTeam()) == 0 and player.getTeam().gameData[
                    'survivalSeconds'] is None:
        ...
        # in teams mode we split up teams
            ...
            if self._soloMode:
                ...
                for team in self.teams:
                    ...
                    while True:
                        playersWithLives = [
                            p for p in team.gameData['spawnOrder']
                            if p.exists() and p.gameData['lives'] >= testLives]
                        if len(playersWithLives) == 0:
                            break
    ...
    def _printLives(self, player):
        ...
        bs.PopupText(
            ...
            color=(1, 1, 0, 1),
            offset=(0, -0.8, 0),
            randomOffset=0.0, scale=1.8, position=pos).autoRetain()
    ...
    def onBegin(self):
        ...
        if self._soloMode:
            self._vsText = bs.NodeActor(bs.newNode("text", attrs={
                'position': (0, 105),
                'hAttach': "center",
                'hAlign': 'center',
                'maxWidth': 200,
        ...
        if (isinstance(self.getSession(), bs.TeamsSession)
            ...
            if self._getTotalTeamLives(
                    self.teams[0]) < self._getTotalTeamLives(
                    self.teams[1]):
            ...
            addIndex = 0
            while self._getTotalTeamLives(lesserTeam) < self._getTotalTeamLives(
                    greaterTeam):
    ...
    def _getLivingTeams(self):
        return [team for team in self.teams if len(team.players) > 0 and
                any(player.gameData['lives'] > 0 for player in team.players)]
