...
##############-------commands-------########
        ...
        #bs.gameTimer(8000,call=self._checkHost,repeat=True)
    #def _checkHost(self):
        #for client in bsInternal._getGameRoster():
           #if client['displayString'] == "\xee\x80\xb0PCID":
               #bs.screenMessage("Host is playing",color = (1,0,1))
    #def _checkChat(self):
...
# if Spaz is dead, count that to the statistics
            bs.statAdd('Player Deaths')
            ...
            if killer is not None and killer.getTeam() is not player.getTeam():
                ...
                if not self.hasEnded():
                    bs.statAdd('Player Kills')
                    self.scoreSet.playerScored(killer,pts,kill=True,victimPlayer=player,importance=importance,showPoints=self._showKillPoints)
                    
            if killer is not None and killer != player and killer.getTeam() == player.getTeam():
                if not self.hasEnded():
                    bs.statAdd('Betrayals')

            if killer is not None and killer == player:
                if not self.hasEnded():
                    bs.statAdd('Suicides')
