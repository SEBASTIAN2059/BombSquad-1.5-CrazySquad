...
class SoccerGame(bs.TeamGameActivity):
    ...
    def getSettings(cls,sessionType):
        return [("Score to Win",{'minValue':1,'default':1,'increment':1}),
                ...
                ("Respawn Times",{'choices':[('Shorter',0.1),('Short',0.5),('Normal',1.0),('Long',2.0),('Longer',4.0)],'default':1.0}),
    ...
    def __init__(self,settings):
        ...
        self._puckModel = bs.getModel("balonBasketball")
        self._puckTex = bs.getTexture("balonBasketballColor")
    ...
    def onBegin(self):
        ...
        try:
            bs.getActivity().getMap().node.materials = [bs.getSharedObject('footingMaterial')]
            bs.getActivity().getMap().floor.materials = [bs.getSharedObject('footingMaterial')]
        except Exception: print('Can\'t unfreeze the map')
        try: bs.getActivity().getMap().floor.color = (1,0.5,0.2)
        except Exception: print('Can\'t change the color map')
