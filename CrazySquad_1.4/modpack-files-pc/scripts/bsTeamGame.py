...
gDefaultTeamColors = ((1*2,1*2,1*2), (0.05, 0, 0.05))
gDefaultTeamNames = ("Angeles", "Demonios")
...
class TeamsScoreScreenActivity(bsGame.ScoreScreenActivity):
    ...
    def showPlayerScores(
        ...
        def _txt(
                ...
                maxWidth=120.0):
            bsUtils.Text(text, color=(0, 0.7, 0.7, 0.5),
        ...
        for i, player in enumerate(playerOrderPrev):
            ...
            if scoreChange > 0:
                ...
                bs.gameTimer(tDelay+delay1, bs.Call(
                    _safeSetAttr, sTxt.node, 'color', (0, 1, 0, 1)))
...
class TeamVictoryScoreScreenActivity(TeamsScoreScreenActivity):
    ...
    def onBegin(self):
        ...
        bsUtils.ZoomText(bestTxt,
                         ...
                         scale=0.25,
                         color=(1*2, 1*2, 1*2, 1.0), jitter=3.0).autoRetain()
    ...
    def _showTeamName(self, posV, team, killDelay, shiftDelay):
        bsUtils.ZoomText(bs.Lstr(value='${A}:', subs=[('${A}', team.name)]),
                         ...
                         flash=False, trail=True, hAlign='right', maxWidth=300,
...
class TeamSeriesVictoryScoreScreenActivity(TeamsScoreScreenActivity):
    ...
    def onBegin(self):
        ...
        bsUtils.Text(txt,
                     vAlign='center',
                     maxWidth=300,
                     color=(0, 1, 0, 1.0), position=(0, 220),
        ...
        if mkp is not None:
            ...
            bsUtils.Text(
                ...
                hAlign='left', scale=0.6, color=(0.3, 0.7, 0.2, 1.0),
                transition='inLeft', transitionDelay=t).autoRetain()
            t += 4*tIncr
            bsUtils.Image(
                mkp.getIcon(),
    ...
    def _showWinner(self, team):
        if not self._isFFA:
            ...
            bsUtils.ZoomText(team.name, position=(0, 97),
                             color=team.color, scale=1.0, jitter=1.0,
