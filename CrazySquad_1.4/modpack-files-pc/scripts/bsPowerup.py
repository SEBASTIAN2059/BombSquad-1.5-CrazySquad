...
class PowerupFactory(object):
    ...
    def __init__(self):
        ...
        self.texSonicBomb = bs.getTexture("powerupEndBombs")
...
class Powerup(bs.Actor):
    ...
    def __init__(self,position=(0,1,0),powerupType='tripleBombs',expire=True,decorate=True, shield=False):
        ...
        if powerupType == 'tripleBombs': 
            tex = factory.texBomb
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionTripleBombs')
        elif powerupType == 'punch': 
            tex = factory.texPunch
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionPunch')
        elif powerupType == 'iceBombs': 
            tex = factory.texIceBombs
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionIceBombs')
        elif powerupType == 'impactBombs': 
            tex = factory.texImpactBombs
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionImpactBombs')
        elif powerupType == 'landMines': 
            tex = factory.texLandMines
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionLandMines')
        elif powerupType == 'stickyBombs': 
            tex = factory.texStickyBombs
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionStickyBombs')
        elif powerupType == 'luckyBlock': 
            tex = factory.texLuckyBlock
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionLuckyBlock')
        elif powerupType == 'shield': 
            tex = factory.texShield
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionShield')
        elif powerupType == 'health': 
            tex = factory.texHealth
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionHealth')
        elif powerupType == 'curse': 
            tex = factory.texCurse
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionCurse') 
        elif powerupType == 'rm': 
            tex = factory.texRM
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionHealth') ####
            ...
            tex = factory.texUS
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionEspectralBomb')
        elif powerupType == 'cyp': 
            tex = factory.texCP
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionSuicide')
        elif powerupType == 'tntBombs': 
            tex = factory.texTntBombs
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionTntBombs')
        elif powerupType == 'magicBomb': 
            tex = factory.texDeathOrb
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionMagicBomb')
        elif powerupType == 'speed': 
            tex = factory.texSpeed
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionSpeed')
        elif powerupType == 'iceImpact': 
            tex = factory.texIceImpact
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionIceImpact')
        elif powerupType == 'endBomb': 
            tex = factory.texSonicBomb
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionEndBomb')
        elif powerupType == 'extraLive': 
            tex = factory.texExtraLive
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionExtraLive')
        elif powerupType == 'powerupBomb': 
            tex = factory.texPowerupBomb
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionPowerupBomb')
        elif powerupType == 'power': 
            tex = factory.texPower
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionPower')
        elif powerupType == 'bombJumping': 
            tex = factory.texBombJumping
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionBombJumping')
        elif powerupType == 'upSticky': 
            tex = factory.texUpSticky
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionUpSticky')
        elif powerupType == 'electricBomb': 
            tex = factory.texElectricBomb
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionElectricBomb')
        elif powerupType == 'powerupsLol': 
            tex = factory.texPowerupsLol
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionPowerupsLol')
        elif powerupType == 'god': 
            tex = factory.texGod
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionGod')
            ...
            tex = factory.texBB
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionBugBomb')
        elif powerupType == 'eggBomb': 
            tex = factory.texEgg
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionEggBomb')
        elif powerupType == 'shockBomb': 
            tex = factory.texShock
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionShockBomb')
        elif powerupType == 'dirigida': 
            tex = factory.texDirigida
            color = (1.2,1.2,1)
            powerupName = bs.Lstr(resource='descriptionDirigida')
        ...
        # animate in..
        curve = bs.animate(self.node,"modelScale",{0:0,140:1.6,200:0.9})
        bs.gameTimer(200,curve.delete)
        
        if settings.powerup_Shield == 0:
            None
        elif settings.powerup_Shield == 1:
            self.shield = bs.newNode('shield', owner=self.node,
                                          attrs={'color':(random.random()*2,random.random()*2,random.random()*2) ,'radius':0.95})  #'volumeIntensityScale': 1.0,'intensity':0.6,'radius':0.25})
            self.node.connectAttr('position', self.shield,'position')                                              
        elif settings.powerup_Shield == 2:
            self.shield = bs.newNode('light', owner=self.node,
                                          attrs={'color':(random.random()*2,random.random()*2,random.random()*2) ,'volumeIntensityScale': 0.7,'intensity':0.3,'radius':0.20})
            self.node.connectAttr('position', self.shield,'position') 
        elif settings.powerup_Shield == 3:
            m = bs.newNode('math', owner=self.node, attrs={'input1': (0, 0.5, 0), 'operation': 'add'})
            self.node.connectAttr('position', m, 'input2')
            self._powText = bs.newNode('text',
                                      owner=self.node,
                                      attrs={'text':powerupName,
                                             'inWorld':True,
                                             'shadow':1.0,
                                             'flatness':1.0,
                                             'color':(1,1,1),
            ...
            bs.animate(self._powText, 'scale', {0: 0.0, 100: 0.01})	  
