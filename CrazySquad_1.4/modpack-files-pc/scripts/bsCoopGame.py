    ...
    def showUI(self):
        ...
        if self._showFriendScores:
            gcButton = bs.buttonWidget(
                parent=d, color=gButtonsColor, textColor = gTextColor, 
        ...
        menuButton = bs.buttonWidget(
            parent=d,color=gButtonsColor, autoSelect=True, position=(hOffs - 130 - 60, vOffs),
            size=(110, 85),
        ...
        self._restartButton = restartButton = bs.buttonWidget(
            parent=d,color=gButtonsColor, autoSelect=True, position=(hOffs - 60, vOffs),
            size=(110, 85),
        ...
        if showNextButton:
            ...
            nextButton = bs.buttonWidget(
                ...
                size=(110, 85),
                label='', onActivateCall=call, color=gButtonsColor,
    ...
    def _showWorldRank(self, offsX):
        ...
        if self._doNewRating:
            ...
            bsUtils.ZoomText(
                ...
                flash=True, trail=False, scale=0.6 if available else 0.333,
                tiltTranslate=0.11, hAlign='center',
                position=(190 + offsX, -94),color=gTextColor, 
                maxWidth=200, jitter=1.0).autoRetain()
    ...
    def _showScoreVal(self, offsX):
        bsUtils.ZoomText(
            (str(self._score)
             if self._scoreType == 'points' else bsUtils.getTimeString(
                 self._score * 10)),
            maxWidth=300, flash=True, trail=False, scale=1.0
            if self._scoreType == 'points' else 0.6, hAlign='center',
            tiltTranslate=0.11, position=(190 + offsX, 115),color=gTextColor, 
            jitter=1.0).autoRetain()
        bsUtils.Text(
            bs.Lstr(
                value='${A}:',
...
def fillOutCampaigns():
    ...
    c.addLevel(
        bs.Level(
            ...
            previewTexName='doomShroomPreview'))
    c.addLevel(bs.Level('Final CSMP (Beta)', displayName='Final CSMP (Beta)', gameType=bsFinalCSMP.FinalCSMPGameBeta, settings={}, previewTexName='courtyardPreview'))
    c.addLevel(
        bs.Level(
