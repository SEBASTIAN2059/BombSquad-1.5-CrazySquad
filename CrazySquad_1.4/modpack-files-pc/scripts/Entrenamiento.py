...
def bsGetGames():
    return [EntrenamientoGame]

def bsGetLevels():
    return [bs.Level('Entrenamiento',displayName='${GAME}',gameType=EntrenamientoGame,settings={},previewTexName='rampagePreview'),
bs.Level('Entrenamiento Epic',displayName='${GAME}',gameType=EntrenamientoGame,settings={'Epic Mode':True},previewTexName='rampagePreview')]

class EntrenamientoGame(bs.TeamGameActivity):
    ...
    def getDescription(cls,sessionType):
        return 'Entrena para que no seas un NOOB!.\nCon Color KAWAI!!!.'
    ...
    def getSettings(cls,sessionType):
        return [("Epic Mode",{'default':False}),
                       ("Max Bots",{'minValue':1,'maxValue':20,'default':3,'increment':1}),
    ...
    def onTransitionIn(self):
        ...
        if self.settings:
            bs.getSharedObject('globals').tint=(0.2*2,0.8,0.8)
    ...
    def onBegin(self):
        ...
        for i in range(self.settings['Max Bots']):
         bPos = (-5.0+10*random.random(),4.3,-5.5+2.1*random.random())
    ...
    def _onSpazBotDied(self,DeathMsg):
        bPos= (-5.0+10*random.random(),4.4,-5.5+2.1*random.random())
