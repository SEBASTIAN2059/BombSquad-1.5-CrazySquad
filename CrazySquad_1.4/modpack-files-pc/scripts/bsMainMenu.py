            ...
            if bs.getLanguage() == 'Chinese':
                ...
                for shadow in (True, False):
                    ...
                    self._makeWord('d', x if bsUI.gSmallUI else x-30 if bsUI.gMedUI else x-25, y+yExtra-10 if bsUI.gSmallUI else y-23+0.8*yExtra+60 if bsUI.gMedUI else y-23+0.8*yExtra, delay=delay,
                                   ...
                                   shadow=shadow)
                    #DIVISION
                    self._makeWord2('M', x-10 if bsUI.gSmallUI else x-50 if bsUI.gMedUI else x-45, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, scale=0.36*gScale,
                                   delay=delay, vrDepthOffset=14, shadow=shadow)
                    x += spacing
                    delay += delayInc
                    self._makeWord2('o', x+3 if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.35*gScale,
                                   shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                    self._makeWord2('d', x if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.35*gScale,
                                   vrDepthOffset=7, shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                    self._makeWord2('P', x if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.36*gScale,
                                   shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                    self._makeWord2('a', x if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.35*gScale,
                                   vrDepthOffset=7, shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                    self._makeWord2('c', x if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.35*gScale,
                                   shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                    self._makeWord2('k', x if bsUI.gSmallUI else x-40 if bsUI.gMedUI else x-25, y-125+0.7*yExtra if bsUI.gSmallUI else y+0.7*yExtra if bsUI.gMedUI else y-55+0.7*yExtra, delay=delay, scale=0.35*gScale,
                                   vrDepthOffset=7, shadow=shadow)
                    x += spacing*0.9
                    delay += delayInc
                self._makeLogo(baseX+450 if bsUI.gSmallUI else baseX+420 if bsUI.gMedUI else baseX+435, y+1.0*yExtra  if bsUI.gSmallUI else y+55+1.0*yExtra if bsUI.gMedUI else y-45+1.0*yExtra, 0.08*gScale,
                               delay=baseDelay)
    ...
    def _makeWord2(self, word, x, y, scale=1.0, delay=0,
        ...
        if shadow:
            wordShadowObj = bs.NodeActor(bs.newNode('text', attrs={
                'position':(x,y),
                'big':True,
                'color':random.choice([(0,1,1),(0,0.9,1),(0,0.8,1),(0,0.7,1),(0,0.6,1),(0,0.5,1)]) if settings.theme == 'themeCSMP' else (0.5,0.5,0.5),
                'tiltTranslate':0.09,
                'opacityScalesShadow':False,
                'shadow':0.4,
                'vrDepth':-130,
